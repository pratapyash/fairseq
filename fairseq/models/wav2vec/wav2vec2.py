...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, x, padding_mask=None, layer=None, return_attention=True):
        if return_attention:
            x, layer_results, attention_weights = self.extract_features(x, padding_mask, layer, return_attention=return_attention)
        else:
            x, layer_results = self.extract_features(x, padding_mask, layer, return_attention=return_attention)

        if self.layer_norm_first and layer is None:
            x = self.layer_norm(x)
        
        if return_attention:
            return x, layer_results, attention_weights
        else:
